% close all;
% load machine data
load('protons_IR1noField-GATE-SG.mat') ;
...
k = 7 ;
LatY = matRad_singleGaussFunction(Y,machine.data(k).meanx, machine.data(k).sigmax) ;
LatZ = matRad_singleGaussFunction(Z,machine.data(k).meany, machine.data(k).sigmay) ;
