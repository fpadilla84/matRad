function [gammaCube,gammaPassRateCell] = matRad_gammaIndex2D(cube1,cube2,resolution,slice,criteria,n,localglobal,cst)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% pad cubes
cubex1 = zeros([size(cubex1c,1)+2*searchX size(cubex1c,2)+2*searchY]);
cubex2 = zeros([size(cubex2c,1)+2*searchX size(cubex2c,2)+2*searchY]);
...
for i = -searchX:searchX
    for j = -searchY:searchY
            ...
            delta_sq = ((i*resolution(1))^2 + ...
                        (j*resolution(2))^2) / dist2AgreeMm^2;                 
...
gammaCubeSqx(cut1,cut2) = gammaCubeSq((1+searchX):(end-searchX), ...
                                           (1+searchY):(end-searchY));
...
if strcmp(localglobal,'local')
    ...
    doseThresholdX(cut1,cut2) = doseThreshold((1+searchX):(end-searchX), ...
                                                   (1+searchY):(end-searchY));
