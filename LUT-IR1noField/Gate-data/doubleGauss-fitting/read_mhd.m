function [img, info]=read_mhd(filePathOrFolderPath, optionalFileName)
if nargin == 2
    if filePathOrFolderPath(end) ~= filesep
        filePathOrFolderPath = [filePathOrFolderPath, filesep];
    end
    
    filename= [filePathOrFolderPath, optionalFileName];
else
    filename = filePathOrFolderPath;
end
